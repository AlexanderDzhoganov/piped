...
var Piped = function(object) {
    ...
    this.resolve = function(fn) {
        if(!this._lastPromise) {
            return fn(this._object, this.validationErrors);
        }

        return this._lastPromise.then(function(self) {
            return fn(self._object, this.validationErrors);
        });
    }
    ...
    this.scope = function() {
        ...
        var self = new Piped(this._getProp());
        self._ancestor = this;
        return self;
    }
    ...
    this.scopeBack = function() {
        if(this._ancestor) {
            this._ancestor.validationErrors = 
                this._ancestor.validationErrors.concat(this.validationErrors);
    ...
    this.rootScope = function() {
        var self = this;
        while(self._ancestor) {
            self._ancestor.validationErrors = 
                self._ancestor.validationErrors.concat(self.validationErrors);
    ...
    this.castTo = function(type) {
        ...
        if(type === 'integer') {
            ...
            throw 'castTo() encountered unknown type "' + type + '", proped "integer", "float" or "string"';
        }
    ...
    this.rescope = function(path) {
        ...
        while(pathItems.length > 0) {
            ...
            var newScope = new Piped(rootScope._object[key]);
